function varargout = inpsnquad(lat_or_x,lon_or_y,latlim_or_xlim,lonlim_or_ylim,varargin) 
% inpsnquad returns true for points in a north polar stereographic quadrangle. 
% 
...
%% Syntax
% 
%  tf = inpsnquad(lat,lon,latlim,lonlim) 
%  tf = inpsnquad(lat,lon,xlim,ylim)
%  tf = inpsnquad(x,y,latlim,lonlim)
%  tf = inpsnquad(x,y,xlim,ylim)
%  tf = inpsnquad(...,'inclusive')
%  [rows,cols] = inpsnquad(...)
% 
%% Description 
% 
% tf = inpsnquad(lat,lon,latlim,lonlim) returns logical matrix tf, which is
...
% 
% tf = inpsnquad(lat,lon,xlim,ylim) as above, but data limits are defined
...
%
% tf = inpsnquad(x,y,latlim,lonlim) Input data points can be north polar
% stereographic meters. Polar stereographic units are determined based on the
% ranges of inputs with the islatlon function. 
%
% tf = inpsnquad(x,y,xlim,ylim) All inputs can be polar stereographic
% meters. Polar stereographic units are determined based on the
% ranges of inputs with the islatlon function. 
%
% tf = inpsnquad(...,'inclusive') includes points on the edge of the
% polygon defined by xlim,ylim or latlim,lonlim. 
% 
% [rows,cols] = inpsnquad(...) returns rows and columns of lat,lon or x,y that have 
...
% for Geophysics (UTIG), September 2015. Adapted for the northern
% hemisphere in June 2017.
% 
% See also: psngrid, ingeoquad, inpolygon, and find. 
